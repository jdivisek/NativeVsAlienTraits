...
for(q in c("sla.avg.mm2mg.", "HEIGHT.MAX", "Germinule"))
{
  ...
  for(i in 1:3)
  {
    ...
    if(i == 1)
    {
      ...
      if(q == "sla.avg.mm2mg.")
      {
        mtext(expression(SLA~(mm^2/mg)), side=2, line=2.3, outer=F, cex=0.7)
      }
      if(q == "HEIGHT.MAX")
      {
...
NN.TEST <- Diff.mediansNN.full.TEST
NI.TEST <- Diff.mediansNI.full.TEST

#windows(7,9)
#cairo_pdf("Boxplots2.pdf", width=7, height = 9)
tiff("Boxplots2.tif", width=7, height = 9, units="in", res=500, compression = "lzw")
layout.matrix <- matrix(c(rep(1:3,2),
                          rep(4:6,2),
                          rep(7:9,2),
                          rep(19:21,1),
...
###PLOT FIRST GROUP OF HABITATS (T, X, S)--------------------------------------------------------
PlotData <- list(traitT.full[which(traitT.full$INVASION.STATUS != "casual"),c(3:5,2)], 
                 traitX.full[which(traitX.full$INVASION.STATUS != "casual"),c(3:5,2)], 
                 traitS.full[which(traitS.full$INVASION.STATUS != "casual"),c(3:5,2)])
...
###PLOT SECOND GROUP OF HABITATS (M, K, L)--------------------------------------------------------
PlotData <- list(traitM.full[which(traitM.full$INVASION.STATUS != "casual"),c(3:5,2)], 
                 traitK.full[which(traitK.full$INVASION.STATUS != "casual"),c(3:5,2)], 
                 traitL.full[which(traitL.full$INVASION.STATUS != "casual"),c(3:5,2)])
